  ...
  return (
    <div>
      ...
      <form>
        ...
        <label htmlFor="selectPhotos" className="z-[5] flex flex-col border border-dashed rounded-xl justify-center items-center p-6 cursor-pointer select-none">
          ...
          <div className="flex justify-start items-center gap-4">
            {photos.urls.map((photo, index) => (
              <div key={index} className="relative">
                <span onClick={()=> { removePhoto(index) }} className="absolute z-[25] -right-2 -top-2 bg-white hover:bg-gray-200 w-[30px] h-[30px] border rounded-full p-1 flex justify-center items-center text-[12px]">❌</span>
                <img  src={photo} className="w-[90px] h-[90px] border z-[10]" />
              </div>
            ))}
          </div>
